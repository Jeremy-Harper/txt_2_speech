...
def generate_mp3_for_ssml(out_path, filename, ssml, language_code, voice_name):
...
class MP3Generator:
    def __init__(self, input_file, output_dir, language_code, voice_name):
        self.input_file = input_file
        self.output_dir = output_dir
        self.language_code = language_code
        self.voice_name = voice_name
        self.md_filename = output_dir
        self.ssml = ""
        self.mp3_file_list = []
    ...
    def generate_mp3_files(self):
        with open(self.input_file, "r") as md_file:
            for id, line in enumerate(md_file.readlines()):
                line = process_line(line)

                if len(self.ssml.encode("utf-8")) + len(line.encode("utf-8")) > 4500:
                    filename = f'{os.path.basename(self.input_file)[:-3]}-{id}.mp3'  # Assuming .md extension
                    mp3_file = generate_mp3_for_ssml(self.output_dir, filename, self.ssml, self.language_code, self.voice_name)
                                                     self.ssml)
